...
def main():
    ...
    out_class_csv_file = osp.join(args.out_dir, 'class_dict.csv')
    ...
    for label_file in glob.glob(osp.join(args.in_dir, '*.json')):
        ...
        with open(label_file) as f:
            ...
            if rgb_info not in rgb_dict:
                rgb_dict.append(rgb_info)
    ...
    fo = open(out_class_csv_file, 'w', newline='')
    writer=csv.writer(fo)
    writer.writerow(["_background_", "0", "0", "0"])
    m = len(rgb_dict)
    for i in range(m):
